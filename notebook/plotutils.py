...
def scatterplot_matrix(X, names=None, plotter=density_plot, hist_plotter=matplotlib.pyplot.hist, bins=None, hbins=None, tick_count=None, tick_fmt=None, rc=None, **kwargs):
    ...
    for i in range(n):
        for j in range(n):
            ...
            if i == j:
                ...
                hist_plotter(X[:,j], bins=hbins[i] if hbins else 150)
...
def violin_density_plot(X, bins, vbins=100, cmap=matplotlib.cm.gray,**kwargs):
...
def violin_contour_plot(X, bins):
    H2 = violin_density_plot(X, bins, interpolation="nearest", cmap=matplotlib.cm.gist_earth)
